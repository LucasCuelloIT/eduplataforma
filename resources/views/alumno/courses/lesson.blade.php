<x-app-layout>
    ...
    <div style="padding: 24px;">
        <div style="max-width: 1100px; margin: 0 auto;">
            ...
            {{-- Layout de dos columnas si hay video y contenido --}}
            ...
            @else
                ...
                @if($lesson->contenido)
                    <div style="background: white; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); overflow: hidden; margin-bottom: 24px;">
                        ...
                        <div style="padding: 24px; color: #4b5563; line-height: 1.8;">
                            {!! $lesson->contenido !!}
                        </div>
                    </div>
                @endif
                            ...
                            <div style="background: linear-gradient(135deg, #2563eb, #0ea5e9); color: white; border-radius: 16px; padding: 20px; margin-bottom: 24px; display: flex; align-items: center; gap: 20px;">
                                ...
                                <div style="text-align: right;">
                                    @if($nota >= 9)
                                        ...
                                        <p style="font-size: 0.85rem; opacity: 0.9;">¡Sobresaliente!</p>
                                    @elseif($nota >= 7)
                                        <p style="font-size: 2rem;">😄</p>
                                        <p style="font-size: 0.85rem; opacity: 0.9;">¡Muy bien!</p>
                                    @elseif($nota >= 5)
                                        <p style="font-size: 2rem;">💪</p>
                                        <p style="font-size: 0.85rem; opacity: 0.9;">¡Buen intento!</p>
                                    @else
                                        <p style="font-size: 2rem;">📚</p>
                                        <p style="font-size: 0.85rem; opacity: 0.9;">¡Seguí estudiando!</p>
                                    @endif
                                </div>
                            </div>
                        @endif
    ...
    @foreach($exercises as $exercise)
        <div class="ejercicio-card" id="ejercicio-{{ $loop->index }}" style="display: {{ $loop->first ? 'block' : 'none' }}; border: 2px solid #e5e7eb; border-radius: 16px; padding: 20px; margin-bottom: 16px; transition: all 0.3s;
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                <div style="display: flex; gap: 6px;">
                    @foreach($exercises as $dot)
                        <div class="dot-progress" id="dot-{{ $loop->index }}" style="width: 10px; height: 10px; border-radius: 50%; background: {{ $loop->index === $exercise->getKey() - $exercises->first()->getKey() ? '#8b5cf6' : '#e5e7eb' }};"></div>
                    @endforeach
                </div>
...
function irA(index) {
    ...
    window.scrollTo({ top: document.getElementById('ejercicios-form').offsetTop - 100, behavior: 'smooth' });
                            ...
                            <button type="submit" style="background: linear-gradient(135deg, #8b5cf6, #ec4899); color: white; padding: 14px 32px; border-radius: 12px; font-weight: 800; font-size: 1rem; border: none; cursor: pointer; width: 100%; font-family: 'Nunito', sans-serif;">
                                📨 Enviar respuestas
                            </button>
